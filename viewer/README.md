...
- **Objects** drawn where the crate places them, coloured by type: a box or a
  ...
  restrictions. An object's materials, its surface, friction and roughness,
  are listed in its readout too, and so is each `<userData>` code and
  value. Hover an object for its type,
